...
## 2026-06-05 — CallMeBot 503 Service Unavailable for 2 consecutive routines (distinct from 403 WAF pattern)
- **Pattern:** Thu 6/4 13:55Z 01-pre-market `send_long_routine_message` (2 parts attempted) → first POST HTTP 503; 0/2 delivered. Fri 6/5 12:16Z 01-pre-market `send_long_routine_message` (4 parts attempted, safe_part_len=600, inter_part_sleep=60) → first POST HTTP 503 at retry-exhausted Tenacity boundary; 0/4 delivered. **2 consecutive routines, same status code, same first-part failure mode, distinct payloads** — Thu's body was 2,414 chars / 2-part split; Fri's body was 1,716 chars / 4-part split. The L5 lesson hypothesized 403 (WAF length+content composite); these are 503 (server unavailable). Different failure mode.
- **Lesson:** CallMeBot's upstream may be experiencing a sustained service interruption. Distinguishing the two patterns matters: 403 = client-side payload issue (mitigate by shortening / re-ordering / fewer parts); 503 = service-side outage (no client-side mitigation works; routine spec correctly says "do NOT retry within same routine"). The 503 may also be a more aggressive rate-limit response rather than a true outage — CallMeBot doesn't expose the distinction. **Operational implication**: Robin is currently UNREACHABLE via the documented WhatsApp channel for ~36 hours of book activity. Critical info (AVGO Core stop Thu, MSFT 2.46% cushion record, RL td10 today with NFP/LULU compound risk, Polygon 11th block, NFP 13:30Z direction-of-day) is all sitting in `memory/daily/YYYY-MM-DD.md` files — visible via GitHub PR diff, not pushed to phone. **Inbox.md is currently the only reliable bidirectional channel.**
- **Encoded as rule?** No — recorded here. Recommended actions if pattern continues 3rd routine (this Fri's 02-market-open at 14:30Z):
  - Treat WhatsApp as DEGRADED until 503 stops; rely on `memory/daily/YYYY-MM-DD.md` + Notion (bull-personal mirror) as primary surface for Robin.
  - Surface in next 06-weekly-review (tonight 21:30Z) for KW 23 EOW retrospective with both 6/4 + 6/5 outage data points.
  - If 503 persists past Mon 6/8, escalate to Robin via inbox.md preamble asking him to investigate CallMeBot status (account quota, subscription state, alternative provider Twilio/Telegram).
- **First-attempt mitigation (Fri 6/5)**: lowered safe_part_len 700→600 + inter_part_sleep 35→60 per L5 lesson; failure still occurred at 503 on first POST. Confirms 503 is NOT a payload-size / rate-limit issue solvable client-side.
